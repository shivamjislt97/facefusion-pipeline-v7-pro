...
AUTO_SLEEP_ENABLED = bool(AUTO_SLEEP_CFG.enabled)
...
def _dashboard_register_session(chat_id, video_link, queue_job_id):
    ...
    try:
        ...
        url = f"{public_base}/job/{token}" if public_base else f"/job/{token}"
        return token, url
    except Exception as e:
...
def _verify_dashboard_url(url: str, timeout: float = 4.0) -> str:
    """Return url if /healthz responds 200, else empty string."""
    if not url:
        return ""
    try:
        # Derive healthz URL from base (strip /job/token suffix if present)
        base = url.split("/job/")[0].rstrip("/")
...
def _dashboard_start_server_if_enabled():
    ...
    try:
        ...
        # Try cloudflared tunnel first for a real public URL
...
def _validate_worker_completion_receipt(worker_result):
...
async def recover_sleep_countdown_from_state(app):
    ...
    logger.info("startup cleared stale sleep countdown state (resume disabled)")
    ...
    if queues:
        for key, items in queues.items():
            ...
            if valid:
                ...
                restored += len(valid)
    # Session job counter resets to 0 on every restart — user-facing job IDs start at 1 each session
    queue_job_seq.clear()
    logger.info("[QUEUE_RESTORED] jobs=%s session_job_counter=reset", restored)
...
async def _run_queue_worker(context, chat_id):
    ...
    try:
        while True:
            ...
            def _watchdog_stage_limit_sec(stage_name: str, st_watch: dict) -> int:
                ...
                base_limits = {
                    ...
                    "PROCESSING": int(os.environ.get("PIPELINE_WATCHDOG_PROCESSING_SEC", "60") or 60),
            ...
            try:
                ...
                if worker_result:
                    ...
                    if completion_markers:
                        ok_receipt, receipt_reason = _validate_worker_completion_receipt(worker_result)
                        if not ok_receipt:
                            logger.error(
        ...
        if _queue_size(chat_id) == 0 and not _is_chat_busy(chat_id):
            ...
            if app_obj is not None and _is_last_job_auto_sleep_eligible(chat_id):
                try:
...
def list_swap_outputs():
    ...
    if not output_root.exists():
        return []
    return sorted(
        [f for f in output_root.rglob("*_faceswap*") if f.is_file() and f.suffix.lower() in allowed_exts],
...
def normalize_processing_target(target_path, chat_id):
    """Rename target to a glob-safe filename so FaceFusion temp frame pattern matching is reliable."""
    try:
        ...
        if not safe_stem:
            safe_stem = "target"
        chat_suffix = (str(chat_id)[-6:] if chat_id else "job").replace("-", "")
        safe_name = f"{safe_stem}_{chat_suffix}_{int(time.time())}{ext}"
...
def enforce_storage_limit(max_usage_gb=MAX_STORAGE_USAGE_GB, protected_paths=None):
    ...
    stats = clean_workspace("full_clean", protected_paths=protected_paths, keep_latest_outputs=KEEP_LATEST_OUTPUTS)
...
async def _periodic_storage_guard():
    interval = max(300, int(SAFE_CLEANUP_PERIODIC_INTERVAL_SEC))
    while True:
        try:
            await asyncio.sleep(interval)
            await asyncio.to_thread(enforce_storage_limit, MAX_STORAGE_USAGE_GB)
        except asyncio.CancelledError:
            logger.info("Periodic storage guard stopped")
            raise
        except Exception as e:
            logger.warning("Periodic storage guard failed: %s", e)
...
def _mega_api_download_once(link, dest_dir, timeout_limit, stall_limit):
    ...
    out_path = dest / _safe_download_filename("", f"mega_{file_id}.bin")
    tmp_path = out_path.with_suffix(out_path.suffix + ".part")
    if tmp_path.exists():
        with suppress(Exception):
            tmp_path.unlink()
...
def _mega_python_upload(local_path, user, password):
    ...
    folder_node = None
    try:
        for node in session.get_files().values():
            if isinstance(node, dict) and node.get("t") == 1 and node.get("a"):
                attrs = session._decrypt_attr(node.get("a"), session.master_key)
                if str(attrs.get("n", "")) == folder_name:
                    folder_node = node
                    break
    except Exception:
        folder_node = None
    ...
    uploaded = session.upload(local_path, folder_node)
...
async def run_sleep_countdown(app, chat_id, reason_text, delay_seconds=AUTO_SHUTDOWN_DELAY_SEC):
    ...
    try:
        ...
        if ok:
            ...
            logger.info("studio sleep request successful chat=%s info=%s", chat_id, info)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(links) > 1:
        await update.message.reply_text(
            "⚠️ Single-job mode active hai. Ek baar me sirf 1 link bhejo.",
            reply_markup=main_kb(chat_id),
        )
        return
    ...
    # Single-job safety: reject duplicate submissions while one job is active or queued.
    if _single_job_block_reason(chat_id) or _queue_size(chat_id) > 0:
        await update.message.reply_text(
            "⚠️ Job already running. Please wait.",
            reply_markup=main_kb()
        )
        return
...
async def _download_prepare_face(face_link, chat_id, stage_callback=None, apply_as_default=False):
    ...
    if apply_as_default:
        ...
        clean = face_to_clean_jpg(str(best))
...
async def on_job_completed(app, chat_id, success=True):
    ...
    if app is None:
        logger.info("on_job_completed skipped chat=%s (no application object)", chat_id)
        # Still mark dashboard completion so the web page shows the right state
        # even when the worker subprocess invokes this with app=None.
        try:
...
async def run_pipeline(context, chat_id, video_link, face_link=None, job_mode="direct", progress_seed_message_id=None, queue_job_id=None, gender_mode_override=None, stage_event_hook=None):
    ...
    async def progress(msg, force=False):
        ...
        async with progress_lock:
            ...
            try:
                ...
                raw_wait = _retry_after_seconds(e)
                local_wait = max(5, raw_wait)
                logger.warning("progress edit flood chat=%s retry_after=%ss local_cooldown=%ss", chat_id, raw_wait, local_wait)
                await asyncio.sleep(local_wait)
                return False
            except Exception as e:
    ...
    async def live_update(stage_key, state_text, pct=None, elapsed=None, eta_seconds=None,
        ...
        t = int(total_frames) if total_frames and total_frames > 0 else 0
        if t > 0 and d > t:
            d = t
    ...
    try:
        ...
        out = f"{OUTPUTS_DIR}/{target.stem}_faceswap{out_ext}"
        ...
        async def _run_gpu_retry_attempt(base_cmd, retry_level):
            ...
            while seg_start < (float(duration) - 0.01):
                ...
                seg_out = f"{OUTPUTS_DIR}/{target.stem}_gpu_l{retry_level}_seg{seg_idx}.mp4"
        ...
        if face_map_cfg:
            ...
            for pass_idx, (position_index, mapped_face) in enumerate(sorted(face_map_cfg.items()), start=1):
                pass_out = f"{OUTPUTS_DIR}/{target.stem}_faceswap_sel{pass_idx}{out_ext}"
                pass_cmd = [
            ...
            if clip_frame_ranges:
                ...
                for seg_idx, seg in enumerate(clip_frame_ranges, start=1):
                    seg_out = f"{OUTPUTS_DIR}/{target.stem}_faceswap_seg{seg_idx}.mp4"
                    await live_update(
                        "processing",
                ...
                async def process_progress_line(line):
                    ...
                    # - tqdm style: "...  12%|...| 123/1000 [...]"
                    # - FaceFusion custom tqdm: "extracting: 12% (123/1000)"
